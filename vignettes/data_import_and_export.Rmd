...
using a numeric file id. In `civis`, R objects are serialized using `saveRDS`
for speed and efficiency. Text or csv files created by other programs can also
be read back into R as a data frames.
...
in further calls to any IO function. In the examples that follow, we assume that a default 
...
Finally, `write_civis_file` uploads R objects and files to Amazon S3, which is also
referred to as the 'files endpoint.' R objects saved to the files endpoint expire after
30 days and are serialized using `saveRDS`. R objects can be loaded back into memory by
passing the file id to `read_civis`.
...
To read the file back into memory as a data frame, an appropriate function to convert the
file to a data frame must be provided to the `using` argument of `read_civis`. 
For example, a csv on the files endpoint can be read back into R as a data frame 
by setting `using = read.csv`.
