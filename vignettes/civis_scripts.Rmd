...
Civis Scripts are the way to  _productionize_ your code with Civis Platform. 
...
re-run the same tasks multiple times, it might be time to productionize!
...
Once a container or language script is published as a template, users can create their own instances of the template.
...
To add an output to a job, we can use `scripts_post_r_runs_outputs`. Notice
that the environment variables `CIVIS_JOB_ID` and `CIVIS_RUN_ID` are 
...
This will work for any script type:

```{r, eval = FALSE}
iris <- read_civis(civis_script(run$rId), using = read.csv)
```

A long round trip just to get `iris`!
...
simply update the backing script, using `scripts_patch_r`.
...
If need a link to your template script, use 
